...
export const checkCart = () => {
    ...
    if (!document.querySelector(`.id${product.id}-li`)) {
      ...
      if (document.querySelector("#index")) {
        ...
        li.innerHTML = `
                    ...
                    <span class="id${product.id}-cart-span-money cart-span-money align-self-center text-center">U$S ${(product.amount * product.price).toLocaleString("es-AR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
  ...
  if (!document.querySelector("#checkout") && cartIcon.childElementCount == 0) {
    ...
    document.querySelector("#cart-icon-total") && (document.querySelector("#cart-icon-total").innerText = `U$S ${total.toLocaleString("es-AR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`);
    document.querySelector("#cart-page-total") && (document.querySelector("#cart-page-total").innerText = `U$S ${total.toLocaleString("es-AR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`);
    ...
    document.querySelectorAll(`.id${product.id}-cart-subtract-button`).forEach(element => {
      element.addEventListener("click", () => {
        ...
        document.querySelectorAll(`.id${product.id}-cart-span-money`).forEach(element => {
          element.innerText = `U$S ${(product.amount * product.price).toLocaleString("es-AR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
        });
...
export const fetchPrice = async (total, deliveryPrice) => {
  try {
    ...
    const price = 700; // This value should be manually adjusted regularly
